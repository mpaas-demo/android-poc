...
//        rpcInvokeContext.setGwUrl(gwUrl);
    ...
    private void testGet() {
        ...
        taskService.parallelExecute(new Runnable() {
            ...
            public void run() {
                try {
                    // 设置请求
                    FriendJsonGetReq req = new FriendJsonGetReq();
                    req.name = "123";
                    req.pass = "123";
                    String response = client.friendJsonGet(req);
                    if (response != null) {
    ...
    private void testPost() {
        taskService.parallelExecute(new Runnable() {
            ...
            public void run() {
                try {
                    String response = client.postPost();
                    if (response != null) {
                        showToast(20210715, response);
                    }
                } catch (RpcException e) {
                ...
                try {
                    ...
                    if (userInfo != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(RpcMainActivity.this, RpcMainActivity.this.getString(R.string.limit_request_Data) + userInfo.name + ", age: " + userInfo.age + ", vipInfo expireTime: " + userInfo.vipInfo.expireTime + ", vipInfo level: " + userInfo.vipInfo.level, Toast.LENGTH_SHORT).show();
                            }
                        });
